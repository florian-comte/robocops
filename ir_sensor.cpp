...
void update_ir_sensors(){
  if(millis() >= next_back_ir_capture){
    ir_activated_averaged[TOP_IR] = (digitalRead(IR_SENSORS_PINS[TOP_IR]) + (IR_SENSOR_BACK_WINDOW - 1) * ir_activated_averaged[TOP_IR]) / IR_SENSOR_BACK_WINDOW;
    ir_activated_averaged[BOTTOM_IR] = (digitalRead(IR_SENSORS_PINS[BOTTOM_IR]) + (IR_SENSOR_BACK_WINDOW - 1) * ir_activated_averaged[BOTTOM_IR]) / IR_SENSOR_BACK_WINDOW;

    next_back_ir_capture += IR_SENSOR_BACK_INTERVAL;
  }

  if(millis() >= next_front_ir_capture){
    ir_activated_averaged[FRONT_IR] = (digitalRead(IR_SENSORS_PINS[FRONT_IR]) + (IR_SENSOR_FRONT_WINDOW - 1) * ir_activated_averaged[FRONT_IR]) / IR_SENSOR_FRONT_WINDOW;
